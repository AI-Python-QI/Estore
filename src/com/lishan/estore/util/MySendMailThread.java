	...
	public void run() {
		...
		Session session = Session.getDefaultInstance(p, new Authenticator() {
			...
			protected PasswordAuthentication getPasswordAuthentication() {
				// 用户名可以用QQ账号也可以用邮箱的别名
				PasswordAuthentication pa = new PasswordAuthentication("gaohuiqiemail@163.com", "weimei52155");
				// 后面的字符是授权码，用qq密码不行！！
				return pa;
			}
		});
		...
		try {
			...
			// 邮件信息封装
			// 1发件人
			//msg.setFrom(new InternetAddress("13524568798@163.com"));
			msg.setFrom(new InternetAddress("gaohuiqiemail@163.com"));
			// 2收件人
			msg.setRecipient(RecipientType.TO, new InternetAddress(user.getEmail()));
			// 3邮件内容:主题、内容
			msg.setSubject(user.getName() + ",欢迎Estore账号,请点击链接激活账号");

			// StringBuilder是线程不安全的,但是速度快，这里因为只会有这个线程来访问，所以可以用这个
			StringBuilder sbd = new StringBuilder();
			sbd.append(user.getName() + "<br/>欢迎！请确认此邮件地址以激活您的账号。<br/>");
			sbd.append("<font color='red'><a href='http://127.0.0.1:8080/Estore/users/active?acode="+user.getAcode()+"'");
			sbd.append(">立即激活</a></font><br/>");
			sbd.append("或者点击下面链接:<br/>");
			sbd.append("http://127.0.0.1:8080/Estore/users/active?acode="+user.getAcode()+"<br/>");
